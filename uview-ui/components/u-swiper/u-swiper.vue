<template>
	<view class="u-swiper-wrap" :style="{
		...
		<swiper :current="elCurrent" @change="change" @animationfinish="animationfinish" :interval="interval"
			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list[0].children" :key="index">
				<view class="u-list-image-wrap" @tap.stop.prevent="listClick(index)"
					...
					<view class="u-swiper-card">
						<view class="u-swiper-card-date">
							<text class="month-num">{{item.month_num}}</text>
							<text class="month-eng">{{item.month_eng}}</text>
						</view>
						<view class="u-swiper-card-progress">
							<u-line-progress class="progressBar" active-color="#fff" :percent="10" height="10">
							</u-line-progress>
							<text>{{`${item.hasDailyDay} / ${item.days}`}}</text>
						</view>
						<view class="u-swiper-card-setting">
							<view class="dot">
								...
							</view>
						</view>
					...
					<!-- 					<view v-if="title && item.title" class="u-swiper-title u-line-1" :style="[{
						...
						<text class="u-swiper-title-date">{{ item.date }}</text>
						<text v-bind:class="item.mood"></text>
						<text class="u-swiper-title-word">{{ item.title }}</text>
					</view> -->
				</view>
			</swiper-item>
		</swiper>
...
<script>
	...
	export default {
		...
		mounted() {
			// EventBus.$on('postIndex', function(data) {
			// 	console.log("接受的数据为:" + Object.keys(data));
